...
export function DesktopSidebar({ items }: { items: NavItem[] }) {
  ...
  return (
    <aside className="hidden w-60 shrink-0 flex-col overflow-y-auto border-r border-border bg-white sm:flex">
      <div className="shrink-0 px-5 py-5 text-lg font-semibold tracking-tight text-ink">DMS</div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${
                active ? "bg-primary text-white shadow-sm" : "text-ink-muted hover:bg-surface hover:text-ink"
              }`}
            >
