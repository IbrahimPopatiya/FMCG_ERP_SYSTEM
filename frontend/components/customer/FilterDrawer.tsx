...
// Placeholder options until brand and price-range filtering are wired up to
// the real API — selectable in the UI now, but they don't affect the feed yet.
...
// Category is wired to the real feed; Brand and Price Range are placeholder
// sections for now, selectable in the UI but not wired to the feed until the
// backend supports those filters.
export function FilterDrawer({
  ...
  if (open !== prevOpen) {
    setPrevOpen(open);
    if (open) setDraftCategoryId(selectedCategoryId);
  }
    ...
    <div className={`fixed inset-0 z-40 ${open ? "" : "pointer-events-none"}`}>
      ...
      >
            ...
            <div className="flex flex-col py-1">
              <button
                ...
                className={`flex items-center gap-3 rounded-xl px-3 py-3 text-left text-[15px] font-semibold transition-colors ${
                  draftCategoryId === null ? "bg-primary-soft text-primary" : "text-ink hover:bg-surface"
                }`}
              >
              ...
              {categories.map((category, index) => {
                ...
                return (
                  <button
                    ...
                    } ${!active && index < categories.length - 1 ? "border-b border-border/70" : ""}`}
                  >
                    {category.image ? (
        ...
        >
          <button
            type="button"
            onClick={() => onApply(draftCategoryId)}
