...
export default function AdminOrderDetailPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <TopBar
        title="Order Details"
        subtitle={order.data ? `#${order.data.order_number}` : undefined}
            ...
            <div className="mx-auto flex max-w-3xl flex-col gap-4 p-4 pb-6 sm:p-6">
              ...
              <Card className="flex flex-col gap-3 rounded-2xl">
                ...
                <div className="flex items-center gap-2.5 text-sm text-ink">
                  <PersonIcon className="h-4 w-4 shrink-0 text-ink-muted" />
                  {customer.data?.business_name ?? "Loading…"}
                </div>
                {customer.data?.mobile && (
                  <div className="flex items-center justify-between gap-2.5">
                    <span className="flex items-center gap-2.5 text-sm text-ink">
                      <PhoneIcon className="h-4 w-4 shrink-0 text-ink-muted" />
                      {customer.data.mobile}
                    </span>
                    <a
                      href={`tel:${customer.data.mobile}`}
                      className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-danger-soft text-danger transition-colors hover:bg-danger hover:text-white"
                ...
                {customer.data && (
                  <div className="flex items-start gap-2.5 text-sm text-ink">
                    <PinIcon className="mt-0.5 h-4 w-4 shrink-0 text-ink-muted" />
                    <span>
                          ...
                          {editableStatus ? (
                            <label className="mt-1 flex items-center gap-2 text-xs text-ink-muted">
                              ...
                              <input
                                ...
                                className="h-8 w-16 rounded-lg border border-border px-2 text-xs text-ink outline-none focus:border-primary focus:ring-2 focus:ring-primary-soft"
                              />
                            </label>
                          ) : (
                            <p className="text-xs text-ink-muted">
                              {formatCurrency(item.price)} <span className="text-ink-muted/70">x {item.loaded_qty || item.approved_qty || item.ordered_qty}</span>
                            </p>
                          )}
                        </div>
